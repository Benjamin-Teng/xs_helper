...
- **AC1**：Claude Code 安裝後可用 `/xs`；VS Code／Codex IDE 可用 `$skill-installer` 從 `skills/xs` 安裝後以 `$xs` 使用；Codex CLI 可從 `/plugins` 或直接命令安裝完整 plugin。兩個 manifest 皆識別 `xs-helper@0.7.0` 並載入同一份 `skills/xs/`。
...
- **版本策略**：自 `v0.1.0` 起採 semver；對外變動同步更新兩份 `plugin.json`。**現況：v0.7.0**；`.claude-plugin/marketplace.json` 的 entry 亦同步設 `version`（部分安裝器只讀 marketplace entry 的版本，不讀 manifest；Codex marketplace schema 未查證有此欄位，故 `.agents/plugins/marketplace.json` 不設）。
